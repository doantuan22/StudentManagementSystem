...
public class LecturerSchedulePanel extends BasePanel {
    ...
    private void initComponents() {
        ...
        totalSessionsCard = new DashboardCard("Tổng số buổi dạy", AppColors.STAT_CARD_SECTIONS);
        subjectsCard = new DashboardCard("Số học phần giảng dạy", AppColors.STAT_CARD_SUBJECTS);
    ...
    public void reloadData() {
        try {
            List<Schedule> schedules = scheduleController.getCurrentLecturerSchedules();
            
            // Sắp xếp lịch dạy theo thứ trong tuần và thời gian
    ...
    private JPanel createDayRow(String dayName, List<Schedule> schedules) {
        ...
        // Panel bên phải: Danh sách lớp dạy ngang
        ...
        if (schedules.isEmpty()) {
            JLabel emptyLabel = new JLabel("Không có lịch dạy");
    ...
    /**
     * Hiển thị dialog chi tiết lịch dạy.
     */
    private void showScheduleDetail(Schedule schedule) {
        ...
        addDetailRow(contentPanel, "Sĩ số tối đa", 
                schedule.getCourseSection() == null 
                ? "0" : String.valueOf(schedule.getCourseSection().getMaxStudents()));
